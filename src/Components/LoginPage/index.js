    ...
    <Row  className="m-0 h-100">
      <div className="vertical-center mb-0">
        <form name="form" className="form-login" onSubmit={handleSubmit(onSubmit)}>
          ...
          />
          <small id="emailHelp" className="form-text text-muted">
            {errors.password && errors.password.type === "maxLength" && (<div className="error">longitud maxima alcanzada</div>)}
            {errors.password && errors.password.type === "upperCase" && (<div className="error">Mayuscula</div>)}
            {errors.password && errors.password.type === "lowerCase" && (<div className="error">Minuscula</div>)}
            {errors.password && errors.password.type === "specialChar" && (<div className="error">Caracter especial</div>)}
            {errors.password && errors.password.type === "sequence" && (<div className="error">Se detecto secuencia</div>)}
          </small>
          <Button color="primary" className="mt-4 mb-5" size="lg" block type="submit" disabled={!formState.isValid} >Enter</Button>{' '}
          </div>
        </form>
    </div>
  </Row>
